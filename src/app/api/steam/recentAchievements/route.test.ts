...
jest.mock('@/lib/steamRecentAchievements', () => ({ loadRecentAchievements: jest.fn() }))
...
function makeRequest(query = '') {
  return new NextRequest(`http://localhost:3000/api/steam/recentAchievements${query}`)
}
...
beforeEach(() => {
  ...
  ;(loadRecentAchievements as jest.Mock).mockResolvedValue(LIST)
...
test('returns the recent unlocks in the requested language, cached per player and language', async () => {
  const res = await GET(makeRequest('?lang=spanish'))
  expect((res as unknown as { data: unknown }).data).toEqual(LIST)
  expect(loadRecentAchievements).toHaveBeenCalledWith({ id: '7', steamid: '765', apiKey: 'steam-key' }, 'spanish')
...
test('defaults to English and is never browser-cached', async () => {
  ...
  expect(loadRecentAchievements).toHaveBeenCalledWith(expect.anything(), 'english')
  expect(res.headers.get('Cache-Control')).toBe('private, max-age=0')
})

test('returns 503 when Steam is unavailable', async () => {
  ;(loadRecentAchievements as jest.Mock).mockRejectedValue(new Error('down'))
