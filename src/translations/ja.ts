import { Translations } from './en'

export const ja: Translations = {
  header: {
    signIn: 'ログイン',
    status: 'ステータス',
  },
  sidePanel: {
    welcome: 'ようこそ',
    signIn: 'ログイン',
    userSettings: 'ユーザー設定',
    searchConsole: 'コンソールを検索…',
    noResults: '結果なし',
    streak: 'ストリーク',
  },
  categories: {
    wantToPlay: 'プレイしたい',
    playing: 'プレイ中',
    completed: '完了',
  },
  statusSort: {
    label: '並び替え',
    name: 'A-Z',
    lastPlayed: '最終プレイ',
    percent: '完了率',
    points: 'ポイント',
  },
  statusGrid: {
    label: 'グリッドサイズ',
    cols1: '1列表示',
    cols2: '2列表示',
    cols3: '3列表示',
  },
  mainPage: {
    playing: 'プレイ中',
    noGamesInProgress: '現在プレイ中のゲームはありません',
    noGamesInProgressSub: 'RetroAchievementsでゲームを開始するとここに表示されます',
    seeMore: 'もっと見る...',
    wantToPlay: 'プレイしたい',
    noWantToPlay: 'ウィッシュリストは空です',
    noWantToPlaySub: 'RetroAchievementsでリストにゲームを追加するとここに表示されます',
    completed: '完了',
    normal: 'ノーマル',
    hardcore: 'ハードコア',
    noGamesCompleted: '{tab}モードで完了したゲームはありません',
    noGamesCompletedSub: 'ゲームを完了するとここに表示されます',
  },
  pinnedGames: {
    title: 'Pinned games',
    viewRecentAria: 'View recently played',
    viewPinnedAria: 'View pinned games',
    pinAria: 'Pin game',
    unpinAria: 'Unpin game',
    error: "Couldn't update pin, please try again",
    addAria: 'Pin a game',
    searchPlaceholder: 'Search a game to pin…',
    confirmPin: 'Pin',
  },
  userData: {
    welcome: 'ようこそ',
    email: 'メール',
    theme: 'テーマ',
    totalPoints: '合計ポイント',
    totalSoftcorePoints: 'ソフトコア合計ポイント',
    signInRA: 'RetroAchievementsにログイン',
    signInSteam: 'Steamにログイン',
    admin: '管理者',
    userId: 'ユーザーID',
    steamId: 'Steam ID',
    connectedAccounts: '連携アカウント',
    connected: '連携済み',
    notConnected: '未連携',
    connect: '連携',
    comingSoon: '近日公開',
    steamConnect: 'Steam と連携',
    steamDisconnect: 'Steam の連携を解除',
    steamDisconnecting: '解除中…',
    steamLinked: 'Steam アカウントを連携しました',
    steamAlreadyLinked: 'その Steam アカウントは既に別のユーザーと連携されています',
    steamCancelled: 'Steam のサインインをキャンセルしました',
    steamFailed: 'Steam アカウントを連携できませんでした。もう一度お試しください。',
    gamesTracked: '追跡中のゲーム',
    inProgress: '進行中',
    completedSC: '完了SC',
    completedHC: '完了HC',
    recentAchievementsLoaded: '件の最近の実績を読み込みました',
    location: '場所',
    notSet: '未設定',
    searchCountry: '国を検索...',
    noResults: '国が見つかりません',
    favoriteGame: 'お気に入りゲーム',
    favoriteGameSoon: '検索機能は近日対応予定',
    favoriteGameSearch: 'ゲームを検索…',
    favoriteGameRemove: '削除',
    favoriteGameChange: '変更',
    favoriteGameNoResults: '結果がありません',
    favoriteGameNoGames: 'ライブラリにゲームがありません',
    changePassword: 'パスワード変更',
  },
  userTheme: {
    theme: 'テーマ',
  },
  userConfig: {
    accountSettings: 'アカウント設定',
    changePassword: 'パスワード変更',
    language: '言語',
    signOut: 'ログアウト',
    signOutRA: 'RAからログアウト',
  },
  userCharts: {
    games: 'ゲーム',
    recentAchievements: '最近の実績',
  },
  changePassword: {
    current: '現在のパスワード',
    new: '新しいパスワード',
    confirm: '新しいパスワードを確認',
    minLength: '最低6文字',
  },
  editProfileModal: {
    title: '編集',
    current: '現在の値',
    newLabel: '新しい',
    confirmLabel: '新しいものを確認',
    mismatch: '値が一致しません',
    avatarPreview: 'プレビュー',
    errorGeneric: '問題が発生しました。もう一度お試しください。',
    errorAvatarLoad: 'そのURLから画像を読み込めませんでした',
    success: '正常に更新されました',
    cancel: 'キャンセル',
    save: '保存',
    saving: '保存中...',
    fields: {
      name: 'ユーザー名',
      email: 'メール',
      avatar: 'アバター',
    },
  },
  userStats: {
    globalRank: 'グローバルランク',
    awards: 'アワード',
    total: '合計',
    masteries: 'マスタリー',
    beatenHC: 'クリアHC',
    beatenSC: 'クリアSC',
    completions: '完了数',
    events: 'イベント',
    unlockedHC: 'HCでアンロック',
    unlockedSC: 'SCでアンロック',
    achievementsUnlocked: '実績',
    games: 'ゲーム',
    registeredGames: '登録済み',
    inProgress: '進行中',
    completedSC: '完了SC',
    completedHC: '完了HC',
  },
  gameInfoTable: {
    sortOrder: '順序',
    sortPoints: 'ポイント',
    sortRarity: 'レア度',
    filterAll: 'すべて',
    filterEarned: '獲得済み',
    filterUnearned: '未獲得',
    headerIcon: 'アイコン',
    headerAchievement: '実績',
    headerPlayers: 'アンロック者',
    headerHC: 'HCアンロック',
    headerRarity: 'レア度',
    headerPoints: 'ポイント',
    headerEarned: '獲得',
    missableWarning: '⚠ 取り逃がし注意の実績',
    expandTable: 'すべての実績を表示',
    collapseTable: '折りたたむ',
    author: '作者',
  },
  achievement: {
    progression: 'プログレッション',
    winCondition: '勝利条件',
    missable: '取り逃がし注意',
    completed: '完了',
    players: 'プレイヤー',
    inHardcore: 'ハードコアで',
    haveIt: '%が持っています',
    notEarned: '未獲得',
    earnedOn: 'アンロック日',
    noComments: 'No comments yet',
    by: 'by',
  },
  gameComments: {
    title: 'Game Comments',
    loading: 'Loading comments...',
    empty: 'No comments for this game yet',
    error: 'Could not load comments',
    showMore: 'Show more',
    collapse: 'Collapse',
  },
  gameStatus: {
    mastered: 'Mastered',
    completed: 'Completed',
    beatenHC: 'Beaten HC',
    beaten: 'Beaten',
    inProgress: 'In Progress',
  },
  raLoginModal: {
    title: 'RetroAchievementsにログイン',
    username: 'ユーザー名',
    signIn: 'ログイン',
  },
  authPage: {
    brand: 'CheevoVault',
    tagline: 'CheevoVault',
  },
  loginForm: {
    title: 'ログイン',
    accountCreated: 'アカウントが正常に作成されました',
    invalidCredentials: 'ユーザー名またはパスワードが正しくありません',
    username: 'ユーザー名',
    password: 'パスワード',
    signIn: 'ログイン',
    noAccount: 'アカウントをお持ちでない方は登録してください',
  },
  registerForm: {
    title: '登録',
    username: 'ユーザー名',
    password: 'パスワード',
    invitationCode: '招待コード（該当する場合）',
    createAccount: 'アカウントを作成',
    errorCreating: 'アカウント作成エラー',
    alreadyHaveAccount: 'アカウントをお持ちの方はログイン',
  },
  categoryPage: {
    noWantToPlay: 'このコンソールのリストに何もありません',
    noWantToPlaySub: 'RetroAchievementsでリストにゲームを追加してください',
    noPlaying: 'このコンソールで何もプレイしていません',
    noPlayingSub: 'ゲームを開始するとここに表示されます',
    noCompleted: 'このコンソールで完了したゲームはまだありません',
    noCompletedSub: 'ゲームを完了するとここに表示されます',
    steamComingSoon: 'Steam連携は近日公開予定',
    steamComingSoonSub: '準備中です — お楽しみに',
  },
  statusGameItem: {
    achievements: '実績',
    noPublishedAchievements: '公開された実績はありません',
    pointsTotal: 'ポイント',
    pointsEarned: '獲得ポイント',
  },
  errorBoundary: {
    title: '問題が発生しました',
    subtitle: 'このページの読み込み中に予期しないエラーが発生しました。もう一度お試しください。',
  },
  gameInfoPage: {
    retry: '再試行',
    id: 'ID',
    publisher: '発売元',
    developer: '開発元',
    genre: 'ジャンル',
    released: '発売日',
    achievements: '実績',
    points: 'ポイント',
    hashesCompatible: '対応ハッシュ',
  },
  pieChart: {
    noData: 'データなし',
  },
  profileStats: {
    hardcorePoints: 'ハードコアポイント',
    truePoints: 'トゥーポイント',
    softcorePoints: 'ソフトコアポイント',
    hardcoreRatio: 'ハードコア率',
  },
  profileRa: {
    memberSince: 'メンバー登録日',
    contributions: '貢献',
    achievementsCreated: '作成した実績',
    pointsContributed: '貢献したポイント',
    signIn: 'RetroAchievementsでログイン',
    playingNow: '現在プレイ中',
    viewOnRA: 'RAで見る',
  },
  lineChart: {
    achievementsLast7Days: '過去7日間で{total}件の実績',
    noActivity: '今週の活動なし',
    achievements: '実績',
  },
  gameExpanded: {
    weekly: '週間',
    monthly: '月間',
    achievementsOverTime: '解除された実績',
    noActivity: 'この期間に解除された実績はありません',
    allAchievements: 'すべての実績',
    pinnedAchievements: 'ピン留めした実績',
    pinAchievementAria: '実績をピン留め',
    pinAchievementModalTitle: '実績をピン留め',
    noUnearned: 'ピン留めできる実績がありません',
    unpinAria: 'ピン留め解除',
    loadError: 'ピン留めした実績を読み込めませんでした',
    points: 'ポイント',
    completion: '達成率',
    remaining: '残り',
  },
  profileSt: {
    signIn: 'Steamでログイン',
  },
  progression: {
    recentProgress: '最近の進捗',
  },
  gamesList: {
    achievements: '実績',
  },
  mainStart: {
    welcome: 'ようこそ',
    description: 'RetroAchievementsとSteamの実績と進捗を追跡するアプリ',
  },
  profileAchievements: {
    recentAchievements: '最近の実績',
  },
  favorites: {
    title: 'ピン留めした実績',
    emptyTitle: 'ピン留めした実績はありません',
    empty: 'ゲームページで実績をピン留めするとここで追跡できます',
    addFavorite: '実績をピン留め',
    removeFavorite: 'ピン留めを外す',
  },
  loadingPage: {
    title: '実績を読み込み中',
    subtitle: 'ゲームデータを取得中...',
    wantToPlay: 'ウィッシュリストを読み込み中...',
    playing: 'アクティブなゲームを読み込み中...',
    completed: '完了したゲームを読み込み中...',
    game: 'ゲームデータを読み込み中...',
  },
  search: {
    placeholder: 'ゲームを検索...',
    hint: 'ライブラリを検索',
    noResults: 'ライブラリにゲームが見つかりません',
    libraryOnly: '検索はライブラリを対象としています。ゲームID（例: 12345）を貼り付けると任意のRAゲームを開けます。',
    openById: 'IDでゲームを開く',
    completedHC: '完了HC',
    completedSC: '完了SC',
    inProgress: '進行中',
    wantToPlay: 'プレイしたい',
  },
  dayModal: {
    title: '獲得した実績',
    empty: 'この日の実績はありません',
    achievements: '実績',
  },
  cards: {
    statsActivity: '統計 & アクティビティ',
    dailyAchievements: 'デイリー実績 — 直近7日間',
    activityLabel: 'アクティビティ',
    activityLastDays: '直近{n}日間',
    mostActiveGames: '最も活発なゲーム — 直近30日間',
    rarestUnlocks: '最近の希少なアンロック',
    abandoned: '放置中',
    abandonedIdle: '日以上',
    almostThere: 'もうすぐ (75–99%)',
    almostThereSub: 'ゲームの進捗が25%以内になるとここに表示されます',
    mastered100: 'マスター & 完了 — 100%',
    reorderMasteredAria: 'マスターしたゲームを並び替え',
    reorderMasteredTitle: 'マスターしたゲームを並び替える',
    dragToReorder: 'ドラッグして並び替え',
    close: '閉じる',
    masteryAwards: 'マスタリー & アワード',
    mastered: 'マスター',
    completedSC: '完了SC',
    bestPerformance: 'ベストパフォーマンス',
    navigation: 'ナビゲーション',
    bestWeek: 'ベスト週',
    bestMonth: 'ベスト月',
    bestDay: 'ベスト日',
    weekOf: '{date}週',
    dayOf: '{date}日',
    noData: 'データなし',
    loading: '読み込み中...',
    noGames: 'ゲームなし',
    noCompletedGames: '完了したゲームはまだありません',
    noCompletedGamesSub: 'ゲームをマスターまたは完了するとここに表示されます',
    noRarityData: '希少度データなし',
    noRarityDataSub: 'レアな実績の解除がここに表示されます',
    noAbandonedGames: '放置中のゲームなし',
    noAbandonedGamesSub: '30日以上放置されたゲームがここに表示されます',
    noGamesInProgress: '進行中のゲームなし',
    recentMasteries: '最近のマスタリー',
    trueRatio: 'トゥーレシオ',
    left: '残り',
    masteryBadge: '★ マスター',
    recentlyPlayed: '最近プレイしたゲーム',
    earnedOf: '{total}個中{n}個獲得',
    noEarned: 'まだ実績を獲得していません',
  },
  groups: {
    title: 'グループ',
    newGroup: '新しいグループ',
    editGroup: 'グループを編集',
    deleteGroup: 'グループを削除',
    confirmDelete: 'このグループとそのすべてのゲームを削除しますか？',
    groupTitle: 'タイトル',
    groupTitlePlaceholder: '例: お気に入り',
    description: '説明',
    descriptionPlaceholder: '任意の説明…',
    icon: 'アイコン（絵文字または画像URL）',
    iconPlaceholder: '🎮 または https://…',
    isPublic: '公開',
    create: '作成',
    save: '保存',
    cancel: 'キャンセル',
    addGame: 'ゲームを追加',
    removeGame: '削除',
    noGroups: 'グループはまだありません',
    noGroupsSub: 'グループを作成してゲームを整理しましょう',
    noGames: 'このグループにゲームはありません',
    maxGroups: '最大4グループまで',
    games: 'ゲーム',
    comingSoon: '近日公開',
    searchGames: 'ゲームを検索…',
    filterAll: 'すべて',
    filter0: '未開始',
    filterProgress: '進行中',
    filter100: '完了',
    noGamesFilter: '選択したフィルターに合うゲームがありません',
    clearFilters: 'フィルターをクリア',
  },
  streak: {
    title: 'ストリーク',
    currentStreak: '現在のストリーク',
    bestStreak: '最高ストリーク',
    totalStreaks: 'ストリーク合計',
    days: '日',
    noStreak: 'アクティブなストリークなし',
    noStreakSub: '毎日実績を解除してストリークを開始しよう',
    noData: 'まだデータがありません',
    noDataSub: '実績が増えたら戻ってきてね',
    achievements: '実績',
    andMore: '+{n} 件',
    chartTitle: '最高ストリーク',
    listTitle: '日別内訳',
    mastery: 'マスタリー',
    beaten: 'クリア',
    completions: 'このストリークでの達成',
  },
  pointsStats: {
    today: '今日',
    thisWeek: '今週',
    thisMonth: '今月',
    avgPerDay: '平均/日 (30日)',
    perDay: '実績/日',
    noActivity: '活動なし',
    active: '継続中',
    noStreak: '連続記録なし',
  },
  charts: {
    popularUnplayed: 'Popular — not yet played',
    popularPlayedAll: "You've played everything!",
    failedToLoad: 'Failed to load',
    topTen: 'Top 10 global players',
    yearHeatmapTitle: 'Activity — last 365 days',
    yearHeatmapLoading: 'Loading yearly activity...',
    yearHeatmapActiveDays: 'active days',
    yearHeatmapLess: 'Less',
    yearHeatmapMore: 'More',
    consoleCompletionTitle: 'Completion by console',
    completionDistTitle: 'Completion distribution',
    games: 'games',
    done: 'done',
    consoleBreakdownTitle: 'Rank by console',
  },
  publicProfile: {
    notFound: 'User "{u}" not found on RetroAchievements',
    recentActivity: 'Recent activity',
    viewStreak: 'View streak page',
    searchUsers: 'Search users',
    searchUsersPlaceholder: 'Search RA username…',
    userTab: 'Users',
    gamesTab: 'Games',
    noUserFound: 'No user found with that exact username',
    searchUsersHint: 'Type the exact RA username (min. 3 characters)',
    noSpaces: 'RA usernames have no spaces',
    searchOnRA: 'Search on RetroAchievements',
  },
  steam: {
    profileTabsLabel: 'プロフィール',
    last2Weeks: '過去2週間',
    rarestUnlocked: 'あなたの最もレア',
    noneUnlocked: 'まだ実績を解除していません',
    notOwned: 'Steam ライブラリにありません',
    screenshot: 'スクリーンショット',
    sortDefault: '順序',
    lastPlayed: '最終プレイ',
    perfect: 'パーフェクト',
    neverPlayed: '未プレイ',
    viewOnSteam: 'Steam で見る',
    totalPlaytime: '総プレイ時間',
    partialProgressNote: '一部のゲームの進捗はまだ読み込み中か取得できなかったため、この一覧は不完全な可能性があります',
    minutesShort: '分',
    hoursShort: '時間',
    playtime: 'プレイ時間',
    achievements: '実績',
    noAchievements: '実績なし',
    progressUnknown: '進捗は未読み込み',
    hiddenAchievement: '隠し実績',
    hiddenAchievementDesc: 'プレイを続けて解除しよう',
    earned: '解除済み',
    locked: '未解除',
    achievementsError: '実績を読み込めませんでした',
    retry: '再試行',
    showAchievements: '実績を表示',
    hideAchievements: '実績を隠す',
    nowPlaying: 'プレイ中',
    gamesOwned: '所持ゲーム',
    gamesSection: 'Steam',
    gamesError: 'Steam のゲームを読み込めませんでした',
    privateProfileHint: 'Steam プロフィールが非公開の場合は、Steam のプライバシー設定で「ゲームの詳細」を公開にしてください。',
    noGamesInCategory: 'Steam のゲームはありません',
    profileError: 'Steam プロフィールを読み込めませんでした',
    steamOnlyHint: 'RetroAchievements も連携すると統計とグラフが表示されます',
    connectRa: 'RetroAchievements と連携',
    recentEmpty: '最近プレイした Steam ゲームはありません',
  },
}
