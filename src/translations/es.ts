import { Translations } from './en'

export const es: Translations = {
  header: {
    signIn: 'Iniciar sesión',
    status: 'Estado',
  },
  sidePanel: {
    welcome: 'Bienvenido',
    signIn: 'Iniciar sesión',
    userSettings: 'Ajustes de usuario',
    searchConsole: 'Buscar consola…',
    noResults: 'Sin resultados',
    streak: 'Racha',
  },
  categories: {
    wantToPlay: 'Quiero jugar',
    playing: 'Estoy jugando',
    completed: 'He completado',
  },
  statusSort: {
    label: 'Ordenar',
    name: 'A-Z',
    lastPlayed: 'Última vez jugado',
    percent: '% completado',
    points: 'Puntos',
  },
  statusGrid: {
    label: 'Tamaño de cuadrícula',
    cols1: '1 por fila',
    cols2: '2 por fila',
    cols3: '3 por fila',
  },
  mainPage: {
    playing: 'Estoy jugando',
    noGamesInProgress: 'Nada en progreso',
    noGamesInProgressSub: 'Empieza un juego en RetroAchievements y aparecerá aquí',
    seeMore: 'Ver mas...',
    wantToPlay: 'Quiero jugar',
    noWantToPlay: 'Tu lista de deseos está vacía',
    noWantToPlaySub: 'Añade juegos a tu lista en RetroAchievements y aparecerán aquí',
    completed: 'Completados',
    normal: 'Normal',
    hardcore: 'Hardcore',
    noGamesCompleted: 'Sin completados en modo {tab}',
    noGamesCompletedSub: 'Completa un juego y aparecerá aquí',
  },
  pinnedGames: {
    title: 'Juegos fijados',
    viewRecentAria: 'Ver jugados recientemente',
    viewPinnedAria: 'Ver juegos fijados',
    pinAria: 'Fijar juego',
    unpinAria: 'Quitar fijado',
    error: 'No se pudo actualizar el fijado, inténtalo de nuevo',
    addAria: 'Fijar un juego',
    searchPlaceholder: 'Buscar un juego para fijar…',
    confirmPin: 'Fijar',
  },
  userData: {
    welcome: 'Bienvenido',
    email: 'Email',
    theme: 'Tema',
    totalPoints: 'Puntos totales',
    totalSoftcorePoints: 'Puntos totales softcore',
    signInRA: 'Iniciar sesion en RetroAchievements',
    signInSteam: 'Iniciar sesion en Steam',
    admin: 'Admin',
    userId: 'ID de usuario',
    steamId: 'ID de Steam',
    connectedAccounts: 'Cuentas conectadas',
    connected: 'Conectado',
    notConnected: 'No conectado',
    connect: 'Conectar',
    comingSoon: 'Próximamente',
    steamConnect: 'Conectar Steam',
    steamDisconnect: 'Desconectar Steam',
    steamDisconnecting: 'Desconectando…',
    steamLinked: 'Cuenta de Steam vinculada',
    steamAlreadyLinked: 'Esa cuenta de Steam ya está vinculada a otro usuario',
    steamCancelled: 'Inicio de sesión en Steam cancelado',
    steamFailed: 'No se pudo vincular tu cuenta de Steam. Inténtalo de nuevo.',
    gamesTracked: 'Juegos registrados',
    inProgress: 'En progreso',
    completedSC: 'Completados SC',
    completedHC: 'Completados HC',
    recentAchievementsLoaded: 'logros recientes cargados',
    location: 'Ubicación',
    notSet: 'No establecido',
    searchCountry: 'Buscar país...',
    noResults: 'No se encontraron países',
    favoriteGame: 'Juego favorito',
    favoriteGameSoon: 'Búsqueda disponible próximamente',
    favoriteGameSearch: 'Busca entre tus juegos…',
    favoriteGameRemove: 'Quitar',
    favoriteGameChange: 'Cambiar',
    favoriteGameNoResults: 'Sin resultados',
    favoriteGameNoGames: 'No hay juegos en la librería',
    favoriteGameError: 'No se pudo guardar tu juego favorito. Inténtalo de nuevo.',
    changePassword: 'Cambiar contraseña',
  },
  userTheme: {
    theme: 'Tema',
  },
  userConfig: {
    accountSettings: 'Configuración de cuenta',
    changePassword: 'Cambiar contraseña',
    language: 'Idioma',
    signOut: 'Cerrar sesion',
    signOutRA: 'Cerrar sesion en RA',
  },
  userCharts: {
    games: 'Juegos',
    recentAchievements: 'Logros recientes',
  },
  changePassword: {
    current: 'Contraseña actual',
    new: 'Nueva contraseña',
    confirm: 'Confirmar nueva contraseña',
    minLength: 'Mínimo 6 caracteres',
  },
  editProfileModal: {
    title: 'Editar',
    current: 'Valor actual',
    newLabel: 'Nuevo',
    confirmLabel: 'Confirmar nuevo',
    mismatch: 'Los valores no coinciden',
    avatarPreview: 'Vista previa',
    errorGeneric: 'Algo salió mal. Inténtalo de nuevo.',
    errorAvatarLoad: 'No se pudo cargar la imagen de esa URL',
    success: 'Actualizado correctamente',
    cancel: 'Cancelar',
    save: 'Guardar',
    saving: 'Guardando...',
    fields: {
      name: 'Usuario',
      email: 'Email',
      avatar: 'Avatar',
    },
  },
  userPage: {
    preferences: 'Preferencias',
    library: 'Tu colección',
    favoriteRaGame: 'Juego favorito de RA',
    favoriteSteamGame: 'Juego favorito de Steam',
    viewData: 'Ver datos',
    hideData: 'Ocultar datos',
    editAvatar: 'Cambiar avatar',
    editName: 'Cambiar nombre de usuario',
    raConnectHint: 'Conecta RetroAchievements para seguir aquí tus juegos retro.',
    steamConnectHint: 'Conecta Steam para seguir aquí tu biblioteca y tus logros.',
    psnHint: 'La conexión con PlayStation Network llegará pronto.',
  },
  userStats: {
    globalRank: 'Rango global',
    awards: 'Premios',
    total: 'Total',
    masteries: 'Masterías',
    beatenHC: 'Terminado HC',
    beatenSC: 'Terminado SC',
    completions: 'Completados',
    events: 'Eventos',
    unlockedHC: 'Desbloqueado HC',
    unlockedSC: 'Desbloqueado SC',
    achievementsUnlocked: 'logros',
    games: 'Juegos',
    registeredGames: 'Registrados',
    inProgress: 'En progreso',
    completedSC: 'Completados SC',
    completedHC: 'Completados HC',
  },
  gameInfoTable: {
    sortOrder: 'Orden',
    sortPoints: 'Puntos',
    sortRarity: 'Rareza',
    filterAll: 'Todos',
    filterEarned: 'Obtenidos',
    filterUnearned: 'Sin obtener',
    headerIcon: 'Icono',
    headerAchievement: 'Logro',
    headerPlayers: 'Desbloqueado por',
    headerHC: 'Desbloqueos HC',
    headerRarity: 'Rareza',
    headerPoints: 'Puntos',
    headerEarned: 'Obtenido',
    missableWarning: '⚠ Logros que se pueden perder',
    expandTable: 'Ver todos los logros',
    collapseTable: 'Colapsar',
    author: 'Autor',
  },
  achievement: {
    progression: 'Progresión',
    winCondition: 'Victoria',
    missable: 'Perdible',
    completed: 'Completado',
    players: 'jugadores',
    inHardcore: 'en hardcore',
    haveIt: '% lo tienen',
    notEarned: 'Sin obtener',
    earnedOn: 'Desbloqueado el',
    by: 'de',
    noComments: 'Sin comentarios aún',
  },
  gameComments: {
    title: 'Comentarios del juego',
    loading: 'Cargando comentarios...',
    empty: 'Sin comentarios para este juego',
    error: 'No se pudieron cargar los comentarios',
    showMore: 'Ver más',
    collapse: 'Colapsar',
  },
  gameStatus: {
    mastered: 'Maestría',
    completed: 'Completado',
    beatenHC: 'Superado HC',
    beaten: 'Superado',
    inProgress: 'En progreso',
  },
  raLoginModal: {
    title: 'Iniciar sesion en RetroAchievements',
    username: 'Usuario',
    signIn: 'Iniciar sesion',
  },
  authPage: {
    brand: 'CheevoVault',
    tagline: 'Un sustitutivo a RetroAchievements — integración con logros de Steam en desarrollo para trackear todas tus plataformas juntas',
  },
  loginForm: {
    title: 'Inicia sesión',
    accountCreated: 'Cuenta creada correctamente',
    invalidCredentials: 'Usuario o contraseña incorrectos',
    username: 'Usuario',
    password: 'Contraseña',
    signIn: 'Iniciar sesión',
    noAccount: '¿No tienes cuenta? Regístrate',
  },
  registerForm: {
    title: 'Registro',
    username: 'Nombre de usuario',
    password: 'Contraseña',
    invitationCode: 'Código de invitación (si aplica)',
    createAccount: 'Crear cuenta',
    errorCreating: 'Error al crear la cuenta',
    alreadyHaveAccount: '¿Ya tienes cuenta? Inicia sesión',
  },
  categoryPage: {
    noWantToPlay: 'Nada en tu lista para esta consola',
    noWantToPlaySub: 'Añade juegos a tu lista en RetroAchievements',
    noPlaying: 'Sin juegos activos en esta consola',
    noPlayingSub: 'Empieza un juego y aparecerá aquí',
    noCompleted: 'Sin completados en esta consola',
    noCompletedSub: 'Completa un juego y aparecerá aquí',
    steamComingSoon: 'Integración con Steam próximamente',
    steamComingSoonSub: 'Estamos trabajando en ello — ¡espera novedades!',
    showAllGames: 'Ver los {n} juegos',
    showAll: 'Ver todos los juegos',
    showLess: 'Ver menos',
    searchGames: 'Buscar tus juegos',
    clearSearch: 'Limpiar búsqueda',
  },
  statusGameItem: {
    achievements: 'logros',
    noPublishedAchievements: 'Sin logros publicados',
    pointsTotal: 'puntos',
    pointsEarned: 'puntos obtenidos',
  },
  errorBoundary: {
    title: 'Algo salió mal',
    subtitle: 'Ocurrió un error inesperado al cargar esta página. Puedes intentarlo de nuevo.',
  },
  gameInfoPage: {
    retry: 'Reintentar',
    id: 'ID',
    publisher: 'Editor',
    developer: 'Desarrollador',
    genre: 'Género',
    released: 'Lanzamiento',
    achievements: 'Logros',
    points: 'Puntos',
    hashesCompatible: 'Hashes compatibles',
  },
  pieChart: {
    noData: 'Sin datos',
  },
  profileStats: {
    hardcorePoints: 'Puntos hardcore',
    truePoints: 'Puntos verdaderos',
    softcorePoints: 'Puntos softcore',
    hardcoreRatio: 'Ratio hardcore',
  },
  profileRa: {
    memberSince: 'Miembro desde',
    contributions: 'Contribuciones',
    achievementsCreated: 'Logros creados',
    pointsContributed: 'Puntos aportados',
    signIn: 'Iniciar sesion con RetroAchievements',
    playingNow: 'Jugando ahora',
    viewOnRA: 'Ver en RA',
  },
  lineChart: {
    achievementsLast7Days: '{total} logros en los últimos 7 días',
    noActivity: 'Sin actividad esta semana',
    achievements: 'logros',
  },
  gameExpanded: {
    weekly: 'Semanal',
    monthly: 'Mensual',
    achievementsOverTime: 'Logros desbloqueados',
    noActivity: 'Sin logros desbloqueados en este periodo',
    allAchievements: 'Todos los logros',
    pinnedAchievements: 'Logros pineados',
    pinAchievementAria: 'Pinear un logro',
    pinAchievementModalTitle: 'Pinear un logro',
    noUnearned: 'No quedan logros por pinear',
    unpinAria: 'Despinear',
    loadError: 'No se pudieron cargar los logros pineados',
    points: 'Puntos',
    completion: 'Completado',
    remaining: 'Restantes',
  },
  profileSt: {
    signIn: 'Iniciar sesion con Steam',
  },
  progression: {
    recentProgress: 'Tus progresos recientes',
  },
  gamesList: {
    achievements: 'logros',
  },
  mainStart: {
    welcome: 'Bienvenido',
    description: 'Sigue tus logros en RetroAchievements y Steam desde un solo sitio.',
  },
  profileAchievements: {
    recentAchievements: 'Logros recientes',
  },
  favorites: {
    title: 'Logros fijados',
    emptyTitle: 'Sin logros fijados',
    empty: 'Fija logros en la página del juego para seguirlos aquí',
    addFavorite: 'Fijar logro',
    removeFavorite: 'Quitar logro fijado',
  },
  loadingPage: {
    title: 'Cargando tus logros',
    subtitle: 'Obteniendo datos de juego...',
    wantToPlay: 'Cargando tu lista de deseos...',
    playing: 'Cargando tus juegos activos...',
    completed: 'Cargando tus completados...',
    game: 'Cargando datos del juego...',
  },
  search: {
    placeholder: 'Buscar tus juegos...',
    hint: 'Busca en tu librería',
    noResults: 'No se encontraron juegos en tu librería',
    libraryOnly: 'La búsqueda cubre tu librería. Pega un ID de juego (ej. 12345) para abrir cualquier juego de RA.',
    openById: 'Abrir juego por ID',
    completedHC: 'Completado HC',
    completedSC: 'Completado SC',
    inProgress: 'En progreso',
    wantToPlay: 'Quiero jugar',
    platformAll: 'Todas',
    platformRa: 'RetroAchievements',
    platformSteam: 'Steam',
  },
  dayModal: {
    title: 'Logros conseguidos',
    empty: 'Sin logros este día',
    achievements: 'logros',
  },
  cards: {
    statsActivity: 'Estadísticas & Actividad',
    dailyAchievements: 'Logros diarios — últimos 7 días',
    activityLabel: 'Actividad',
    activityLastDays: 'últimos {n} días',
    mostActiveGames: 'Juegos más activos — últimos 30 días',
    rarestUnlocks: 'Desbloqueos raros recientes',
    abandoned: 'Abandonados',
    abandonedIdle: 'd+ inactivo',
    almostThere: 'Casi (75–99%)',
    almostThereSub: 'Llega al 25% de un juego para verlo aquí',
    mastered100: 'Masterizados & Completados — 100%',
    reorderMasteredAria: 'Reordenar juegos masterizados',
    reorderMasteredTitle: 'Reordena tus juegos masterizados',
    dragToReorder: 'Arrastra para reordenar',
    close: 'Cerrar',
    masteryAwards: 'Maestría & Premios',
    mastered: 'Masterizado',
    completedSC: 'Completados SC',
    bestPerformance: 'Mejor rendimiento',
    navigation: 'Navegación',
    bestWeek: 'Mejor semana',
    bestMonth: 'Mejor mes',
    bestDay: 'Mejor día',
    weekOf: 'semana del {date}',
    dayOf: 'día del {date}',
    noData: 'Sin datos',
    loading: 'Cargando...',
    noGames: 'Sin juegos',
    noCompletedGames: 'Sin juegos completados aún',
    noCompletedGamesSub: 'Masteriza o completa un juego para verlo aquí',
    noRarityData: 'Sin datos de rareza',
    noRarityDataSub: 'Los desbloqueos raros aparecerán aquí',
    noAbandonedGames: 'Sin juegos abandonados',
    noAbandonedGamesSub: 'Los juegos inactivos 30+ días aparecerán aquí',
    noGamesInProgress: 'Sin juegos en progreso',
    recentMasteries: 'Maestrías recientes',
    trueRatio: 'ratio verdadero',
    left: 'restante',
    masteryBadge: '★ maestría',
    recentlyPlayed: 'Jugados recientemente',
    earnedOf: '{n} de {total} conseguidos',
    noEarned: 'Aún no tienes logros',
    steamRarityLabel: 'de jugadores',
    steamPerfect100: 'Juegos perfectos — 100%',
    noSteamPerfectSub: 'Desbloquea todos los logros de un juego para verlo aquí',
    steamProgress: 'Progreso en Steam',
    steamAvgCompletion: 'Completado medio',
    closestToPerfect: 'A punto de ser perfectos',
  },
  groups: {
    title: 'Grupos',
    newGroup: 'Nuevo grupo',
    editGroup: 'Editar grupo',
    deleteGroup: 'Eliminar grupo',
    confirmDelete: '¿Eliminar este grupo y todos sus juegos?',
    groupTitle: 'Título',
    groupTitlePlaceholder: 'ej. Mis favoritos',
    description: 'Descripción',
    descriptionPlaceholder: 'Descripción opcional…',
    icon: 'Icono (emoji o URL de imagen)',
    iconPlaceholder: '🎮 o https://…',
    isPublic: 'Público',
    create: 'Crear',
    save: 'Guardar',
    cancel: 'Cancelar',
    addGame: 'Añadir juego',
    removeGame: 'Eliminar',
    noGroups: 'Sin grupos todavía',
    noGroupsSub: 'Crea un grupo para organizar tus juegos',
    noGames: 'Sin juegos en este grupo',
    maxGroups: 'Máximo 4 grupos permitidos',
    games: 'juegos',
    comingSoon: 'Próximamente',
    searchGames: 'Buscar tus juegos…',
    filterAll: 'Todos',
    filter0: 'Sin empezar',
    filterProgress: 'En progreso',
    filter100: 'Completado',
    noGamesFilter: 'Ningún juego coincide con los filtros',
    clearFilters: 'Limpiar filtros',
    addError: 'No se pudieron añadir algunos juegos. Inténtalo de nuevo.',
    decadeFilterExcludesSteam: 'El filtro de década no aplica a los juegos de Steam',
    mixedPlatforms: 'Incluye juegos de RetroAchievements y Steam',
  },
  streak: {
    title: 'Racha',
    currentStreak: 'Racha actual',
    bestStreak: 'Mejor racha',
    totalStreaks: 'Total rachas',
    days: 'días',
    noStreak: 'Sin racha activa',
    noStreakSub: 'Desbloquea logros cada día para empezar una racha',
    noData: 'Sin datos de racha todavía',
    noDataSub: 'Vuelve cuando tengas más logros',
    achievements: 'logros',
    andMore: '+{n} más',
    chartTitle: 'Mejores rachas',
    listTitle: 'Desglose diario',
    mastery: 'Maestría',
    beaten: 'Completado',
    completions: 'Completados en esta racha',
  },
  pointsStats: {
    today: 'Hoy',
    thisWeek: 'Esta semana',
    thisMonth: 'Este mes',
    avgPerDay: 'Prom/día (30d)',
    perDay: 'logros/día',
    noActivity: 'sin actividad',
    active: 'activo',
    noStreak: 'sin racha',
  },
  charts: {
    popularUnplayed: 'Popular — aún sin jugar',
    popularPlayedAll: '¡Ya has jugado a todo!',
    failedToLoad: 'Error al cargar',
    topTen: 'Top 10 jugadores globales',
    yearHeatmapTitle: 'Actividad — últimos 365 días',
    yearHeatmapLoading: 'Cargando actividad anual...',
    yearHeatmapActiveDays: 'días activos',
    yearHeatmapLess: 'Menos',
    yearHeatmapMore: 'Más',
    consoleCompletionTitle: 'Completado por consola',
    completionDistTitle: 'Distribución de completado',
    games: 'juegos',
    done: 'completado',
    consoleBreakdownTitle: 'Ranking por consola',
  },
  publicProfile: {
    notFound: 'Usuario "{u}" no encontrado en RetroAchievements',
    recentActivity: 'Actividad reciente',
    viewStreak: 'Ver página de racha',
    searchUsers: 'Buscar usuarios',
    searchUsersPlaceholder: 'Buscar usuario de RA…',
    userTab: 'Usuarios',
    gamesTab: 'Juegos',
    noUserFound: 'No se encontró ningún usuario con ese nombre exacto',
    searchUsersHint: 'Escribe el nombre exacto de RA (mín. 3 caracteres)',
    noSpaces: 'Los usernames de RA no tienen espacios',
    searchOnRA: 'Buscar en RetroAchievements',
  },
  steam: {
    level: 'Nivel',
    online: 'En línea',
    offline: 'Desconectado',
    statGames: 'Juegos',
    statPerfect: 'Juegos perfectos',
    statAchievements: 'Logros desbloqueados',
    profileTabsLabel: 'Perfil',
    last2Weeks: 'Últimas 2 semanas',
    rarestUnlocked: 'Tus más raros',
    noneUnlocked: 'Aún no has desbloqueado logros',
    notOwned: 'No está en tu biblioteca de Steam',
    screenshot: 'Captura',
    sortDefault: 'Orden',
    lastPlayed: 'Jugado por última vez',
    perfect: 'Perfecto',
    neverPlayed: 'Sin jugar',
    viewOnSteam: 'Ver en Steam',
    totalPlaytime: 'Tiempo total',
    partialProgressNote: 'El progreso de algunos juegos aún se está cargando o no se pudo obtener, así que esta lista puede estar incompleta',
    minutesShort: 'min',
    hoursShort: 'h',
    playtime: 'Tiempo jugado',
    achievements: 'logros',
    noAchievements: 'Sin logros',
    progressUnknown: 'Progreso no cargado',
    hiddenAchievement: 'Logro oculto',
    hiddenAchievementDesc: 'Sigue jugando para descubrirlo',
    likelyOnline: '¿Online?',
    likelyOnlineHint: 'Probablemente requiere jugar online — estimado a partir de su descripción',
    earned: 'Conseguido',
    locked: 'Bloqueado',
    achievementsError: 'No se pudieron cargar los logros',
    retry: 'Reintentar',
    showAchievements: 'Ver logros',
    hideAchievements: 'Ocultar logros',
    nowPlaying: 'Jugando ahora',
    gamesOwned: 'juegos en propiedad',
    gamesSection: 'Steam',
    gamesError: 'No se pudieron cargar tus juegos de Steam',
    privateProfileHint: 'Si tu perfil de Steam es privado, pon «Detalles de juegos» en Público en la configuración de privacidad de Steam.',
    noGamesInCategory: 'No hay juegos de Steam aquí',
    profileError: 'No se pudo cargar tu perfil de Steam',
    steamOnlyHint: 'Vincula también RetroAchievements para ver estadísticas y gráficas',
    connectRa: 'Conectar RetroAchievements',
    recentEmpty: 'No hay juegos de Steam jugados recientemente',
  },
}
